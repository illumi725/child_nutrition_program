...
class ExcelDuplicateDialogController:
    ...
    def update_delete_button(self):
        ...
        if num_checked == total_rows:
            self._dlg.btn_delete.setEnabled(False)
            self._dlg.lbl_status.setText("⚠ You cannot delete ALL occurrences. At least one row must be retained.")
    ...
    def on_delete(self):
        ...
        if not require_permission(self._dlg, self._dlg._current_user(), "delete_excel_row"):
            return False

        selected_indices = [i for i, chk in enumerate(self._dlg.checkboxes) if chk.isChecked()]
        ...
        self._dlg.rows = [r for r in self._dlg.rows if (r.get('file_path'), r.get('row_number')) not in deleted_file_rows]
        self._dlg.checkboxes = []
        self._dlg.populate_table()

        from PySide6.QtWidgets import QMessageBox as _QMsg
        if errors:
            self._dlg.lbl_status.setText(f"⚠  {deleted} row(s) deleted. {len(errors)} error(s) occurred.")
            _QMsg.warning(self._dlg, "Partial Success", "Some rows could not be deleted:\n" + "\n".join(errors))
        else:
        ...
        if len(self._dlg.rows) <= 1:
            _QMsg.information(self._dlg, "Done", "Duplicate resolved! Only one occurrence remains.")
            self._dlg.accept()
