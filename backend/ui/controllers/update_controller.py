"""Auto-update check, download, and handoff install (R7 — extracted from MainWindow)."""
...
class UpdateController:
    """Wires background and manual update checks to the main window."""
    ...
    def start_background_check(self) -> None:
        # Don't start background update checks during headless tests or when
        # explicitly disabled via environment variable. This prevents threads
        # from running during short-lived smoke tests which can cause crashes
        # on exit (QThread destroyed while running).
