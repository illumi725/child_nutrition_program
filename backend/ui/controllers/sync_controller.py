...
class SyncController:
    ...
    def on_bulk_sync_exact(self):
        ...
        self._win.grid_exact.set_data(
            self._win.exact_matches, self._win.match_columns, action_label="Sync"
        )
        ...
        QMessageBox.information(
            self._win,
            "Bulk Sync Complete",
            f"Successfully synced {success_count} matches.\nFailed/Skipped: {fail_count}",  # noqa: E501
        )
    ...
    def on_bulk_bday_use_excel(self):
        ...
        if (
            not hasattr(self._win, "bday_discrepancies")
            or not self._win.bday_discrepancies
        ):
            return
        ...
        for record in self._win.bday_discrepancies:
            if not record.get("baseline_mismatch"):
                continue
            success = update_birthday_db(
                record["db"]["beneficiary_id"], record["excel"]["birthday"]
            )
            if success:
                record["baseline_mismatch"] = False
                success_count += 1
        ...
        self._win.grid_bday.set_data(
            self._win.bday_discrepancies,
            self._win.match_columns,
            action_label="BirthdayActions",
        )
        QMessageBox.information(
            self._win,
            "Complete",
            f"Successfully applied Excel birthdays to {success_count} records.",
        )
    ...
    def on_bulk_bday_use_db(self):
        ...
        for record in self._win.bday_discrepancies:
            ...
            success = update_excel_birthday(
                record["excel"]["file_path"],
                record["excel"]["row_number"],
                record["db"]["birthday"],
            )
            if success:
                record["baseline_mismatch"] = False
        ...
        QMessageBox.information(
            self._win,
            "Complete",
            f"Successfully applied DB birthdays to {success_count} Excel records.",
        )
    ...
    def on_bulk_name_use_excel(self):
        ...
        if (
            not hasattr(self._win, "name_discrepancies")
            or not self._win.name_discrepancies
        ):
            return
        ...
        self._win.grid_name.set_data(
            self._win.name_discrepancies,
            self._win.name_match_columns,
            action_label="NameActions",
        )
        QMessageBox.information(
            self._win,
            "Complete",
            f"Successfully applied Excel names to {success_count} DB records.",
        )
    ...
    def on_bulk_name_use_db(self):
        ...
        for record in self._win.name_discrepancies:
            ...
            success = update_excel_name(
                record["excel"]["file_path"], record["excel"]["row_number"], db_fullname
            )
            if success:
                record["name_mismatch"] = False
                success_count += 1
        ...
        QMessageBox.information(
            self._win,
            "Complete",
            f"Successfully applied DB names to {success_count} Excel records.",
        )
    ...
    def _handle_grid_action(self, action_name: str, record: dict, action_widget: Any):
        ...
        if not weight or not height:
            QMessageBox.warning(
                self._win, "Missing Data", "Cannot sync: weight or height is missing."
            )
            return
        ...
        if success:
            ...
            QMessageBox.information(
                self._win,
                "Success",
                f"Successfully synced baseline data for {db.get('lastname', '') + ', ' + db.get('firstname', '')}.",  # noqa: E501
            )
        else:
            QMessageBox.warning(
                self._win,
                "Error",
                "Failed to sync baseline data. Check console for details.",
            )
            ...
            if ok and date_str:
                ...
                if success_db and success_ex:
                    ...
                    QMessageBox.information(
                        self._win,
                        "Success",
                        f"Updated both DB and Excel to {date_str}.",
                    )
                else:
                    QMessageBox.warning(
                        self._win,
            ...
            if ok and name_str:
                ...
                if not ln or not fn:
                    QMessageBox.warning(
                        self._win,
                        "Error",
                        "Invalid name format. Please use 'LASTNAME, FIRSTNAME MIDDLENAME'.",  # noqa: E501
                    )
                    return
                success_db = update_name_db(db["beneficiary_id"], ln, fn, mn)
                success_ex = update_excel_name(
                    ex["file_path"], ex["row_number"], name_str
                )
                if success_db and success_ex:
                    record["name_mismatch"] = False
                    action_widget.mark_as_resolved()
                    QMessageBox.information(
                        self._win,
                        "Success",
                        f"Updated both DB and Excel to {name_str}.",
                    )
                else:
                    QMessageBox.warning(
                        self._win,
