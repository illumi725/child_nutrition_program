...
class DBDuplicateDialogController:
    ...
    def load_counts(self):
        ...
        self._dlg.worker.error.connect(
            lambda e: self._dlg.lbl_status.setText(f"Error: {e}")
        )
    ...
    def update_delete_button(self):
        ...
        self._dlg.selected_ids = [
            self._dlg.records_with_counts[i][0]["beneficiary_id"]
        ...
        if num_checked == 1:
            ...
            self._dlg.btn_delete.setText(
                f"🗑  Delete: {rec.get('lastname')}, {rec.get('firstname')} (ID: {rec['beneficiary_id']})"  # noqa: E501
            )
        else:
    ...
    def on_delete(self):
        ...
        if not require_permission(
            self._dlg, self._dlg._current_user(), "delete_beneficiary"
        ):
            return False
        ...
        for b_id in self._dlg.selected_ids:
            rec = next(
                (
                    r
                    for r, _ in self._dlg.records_with_counts
                    if r["beneficiary_id"] == b_id
                ),
                {},
            )
            counts = next(
                (
                    c
                    for r, c in self._dlg.records_with_counts
                    if r["beneficiary_id"] == b_id
                ),
                {},
            )
    ...
    def on_delete_done(self, success, err):
        if success:
            ...
            if len(self._dlg.records_with_counts) <= 1:
                self._dlg._show_message(
                    "Done", "Duplicate resolved! Only one record remains."
                )
                self._dlg.accept()
        else:
            ...
            self._dlg._update_delete_btn()
            self._dlg._show_message(
                "Delete Failed", f"Could not delete record(s):\n{err}"
            )
