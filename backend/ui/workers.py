...
from core.database import fetch_beneficiaries  # noqa: E402
from core.parser import (  # noqa: E402
    parse_form_5a,
    evaluate_match,
    get_best_site_suggestion,
    clean_name,
)
        ...
        try:
            ...
            for i, fp in enumerate(self.file_paths):
                if os.path.exists(fp) and fp.endswith(".xlsx"):
                    self.progress.emit(
                        10 + int(40 * (i / total_files)),
                        f"Parsing {os.path.basename(fp)}...",
                    )
            ...
            for r in db_records:
                bd = str(r["birthday"])
                if bd not in db_by_bday:
                    db_by_bday[bd] = []
                db_by_bday[bd].append(r)

                ln = (
                    clean_name(r.get("lastname")).split(" ")[0]
                    if clean_name(r.get("lastname"))
                    else ""
                )
                if ln and ln not in db_by_lname:
                    db_by_lname[ln] = []
                if ln:
                    db_by_lname[ln].append(r)
            ...
            INTERFILE_CACHE = os.path.join(
                _get_app_base_dir(), "interfile_index_cache.json"
            )
            ...
            if self.root_dir and os.path.isdir(self.root_dir):
                ...
                self.log.emit(
                    f"[Inter-file] Fingerprint: {fingerprint}  |  Cache exists: {os.path.exists(INTERFILE_CACHE)}"  # noqa: E501
                )
                ...
                if os.path.exists(INTERFILE_CACHE):
                    try:
                        ...
                        if cached_fp == fingerprint:
                            ...
                            cache_ok = True
                            self.log.emit(
                                f"[Inter-file] ✅ Loaded cached index ({len(dir_name_index)} unique names)."  # noqa: E501
                            )
                        else:
                            self.log.emit(
                                "[Inter-file] ⚠ Fingerprint mismatch — rebuilding."
                            )
                            self.log.emit(f"[Inter-file]   Saved:   {cached_fp}")
                            self.log.emit(f"[Inter-file]   Current: {fingerprint}")
                    except Exception as ce:
                        self.log.emit(
                            f"[Inter-file] Cache read failed: {ce}. Rebuilding..."
                        )
                ...
                if not cache_ok:
                    ...
                    self.log.emit(
                        f"[Inter-file] Building fresh index from {len(all_files_to_index)} file(s)..."  # noqa: E501
                    )
                    ...
                    for idx, fp in enumerate(all_files_to_index):
                        ...
                        self.progress.emit(
                            pct, f"Indexing {fname} ({idx + 1}/{total_dir})..."
                        )
                                ...
                                dir_name_index[key].append(
                                    {
                                        ...
                                        "raw_name": r.get(
                                            "raw_name",
                                            f"{r.get('lastname', '')}, {r.get('firstname', '')}",  # noqa: E501
                                        ),
                                    }
                                )
                    ...
                    try:
                        cache_data = {
                            ...
                            "index": {
                                f"{k[0]}|{k[1]}": v for k, v in dir_name_index.items()
                            },
                        }
                        with open(INTERFILE_CACHE, "w", encoding="utf-8") as cf:
                            json.dump(cache_data, cf, ensure_ascii=False)
                        self.log.emit(
                            f"[Inter-file] 💾 Index saved ({len(dir_name_index)} unique names)."  # noqa: E501
                        )
            ...
            for key, group in excel_groups.items():
                if len(group) > 1:
                    ...
                    dup_type = (
                        "Intra-file (Same File)"
                        if len(unique_files) == 1
                        else "Inter-file (Across Selected)"
                    )
                    file_names = [os.path.basename(f) for f in unique_files]
                    excel_duplicates.append(
                        {
                            "name": f"{key[0]}, {key[1]}",
                            "birthday": "Multiple/Conflicting"
                            if len(set(str(r["birthday"]) for r in group)) > 1
                            else str(group[0]["birthday"]),
                            "type": dup_type,
            ...
            # dir_name_index covers ALL files; filter out selected ones to find OTHER files  # noqa: E501
            for key, sel_files in selected_name_index.items():
                if key in seen_dup_keys:
                    continue
                all_index_rows = dir_name_index.get(key, [])
                # index rows are dicts; filter to only those from OTHER (non-selected) files  # noqa: E501
                other_rows = [
                    r
                ...
                if other_rows:
                    ...
                    sel_file_names = list(
                        {os.path.basename(r.get("file_path", "")) for r in sel_rows}
                    )
            ...
            processable_records = [
                ex for ex in excel_records if ex["stable_id"] not in conflict_stable_ids
            ]
            ...
            for i, ex in enumerate(processable_records):
                if i % 10 == 0:
                    self.progress.emit(
                        80 + int(20 * (i / max(1, total_records))),
                        f"Evaluating match {i}/{total_records}...",
                    )

                best_score, best_db_match = 0, None
                ex_bd = str(ex["birthday"])
                ex_ln = (
                    clean_name(ex["lastname"]).split(" ")[0]
                    if clean_name(ex["lastname"])
                    else ""
                )
                ...
                if best_db_match:
                    best_db_match["suggested_site"] = get_best_site_suggestion(
                        ex, sites_cache
                    )
            ...
            # Determine which sites are being scanned by looking at the DB records we successfully matched  # noqa: E501
            ...
            # If 20 people matched beneficiaries in "Site A", we are definitely scanning "Site A".  # noqa: E501
            ...
            for r in db_records:
                if not r.get("_matched"):
                    # Only flag as missing if the database record belongs to one of the sites we just scanned  # noqa: E501
            ...
            self.finished.emit(
                {
                    ...
                    "missing_in_excel": missing_in_excel[
                        :500
                    ],  # Cap at 500 to prevent UI lag if massive
...
class GlobalStatsWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress.emit(
                10, f"Found {len(excel_files)} Excel files. Fetching database..."
            )
            ...
            db_duplicates = sum(
                1 for group in db_name_groups.values() if len(group) > 1
            )
            ...
            excel_duplicates_list = [
                r for group in ex_name_groups.values() if len(group) > 1 for r in group
            ]
            excel_duplicates = len(
                set((r["lastname"], r["firstname"]) for r in excel_duplicates_list)
            )
            ...
            db_lookup = {
                (
                    clean_name(r["lastname"]).split(" ")[0],
                    clean_name(r["firstname"]),
                ): True
                for r in db_records
                if r.get("lastname") and r.get("firstname")
            }
            ex_lookup = {
                (
                    clean_name(ex["lastname"]).split(" ")[0],
                    clean_name(ex["firstname"]),
                ): True
                for ex in all_excel_records
                if ex.get("lastname") and ex.get("firstname")
            }
            ...
            for r in db_records:
                ln = (
                    clean_name(r["lastname"]).split(" ")[0] if r.get("lastname") else ""
                )
            ...
            db_duplicates_list = [
                r for group in db_name_groups.values() if len(group) > 1 for r in group
            ]
            ...
            try:
                with open(GlobalStatsWorker.CACHE_PATH, "w", encoding="utf-8") as f:
                    json.dump(
                        GlobalStatsWorker._serialize(result), f, ensure_ascii=False
                    )
