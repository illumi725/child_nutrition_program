...
class ActionWidget(QWidget):
    ...
    def mark_as_synced(self):
        ...
        self.btn_sync.setStyleSheet(
            "color: #27ae60; font-weight: bold; background: transparent; border: none;"
        )
...
class BirthdayActionWidget(QWidget):
    action_clicked = Signal(
        str, dict, QWidget
    )  # action_name (use_excel, use_db, manual), record_data, widget
    ...
    def __init__(self, record_data):
        ...
        self.btn_excel.clicked.connect(
            lambda: self.action_clicked.emit("use_excel", self.record_data, self)
        )
        self.btn_db.clicked.connect(
            lambda: self.action_clicked.emit("use_db", self.record_data, self)
        )
        self.btn_manual.clicked.connect(
            lambda: self.action_clicked.emit("manual", self.record_data, self)
        )
...
class MissingDbActionWidget(QWidget):
    ...
    def __init__(self, record_data):
        ...
        self.btn_add.clicked.connect(
            lambda: self.action_clicked.emit("add_to_db", self.record_data, self)
        )
...
class MissingExcelActionWidget(QWidget):
    ...
    def __init__(self, record_data):
        ...
        self.btn_delete.clicked.connect(
            lambda: self.action_clicked.emit("delete_from_db", self.record_data, self)
        )
...
class ManageDupWidget(QWidget):
    ...
    def __init__(self, record_data):
        ...
        btn.setStyleSheet(
            "background-color: #c0392b; color: white; font-weight: bold; padding: 3px 8px; border-radius: 3px;"  # noqa: E501
        )
        btn.clicked.connect(
            lambda: self.action_clicked.emit("manage", self.record_data, self)
        )
        ...
        for row_idx, record in enumerate(results_list):
            ...
            if action_label:
                ...
                if action_label == "BirthdayActions":
                    ...
                    action_widget.action_clicked.connect(
                        lambda action, rec, aw: self.action_triggered.emit(
                            action, rec, aw
                        )
                    )
                    ...
                    action_widget.sync_clicked.connect(
                        lambda rec, aw, lbl=action_label: self.action_triggered.emit(
                            lbl, rec, aw
                        )
                    )
            ...
            for col_idx, col in enumerate(columns):
                ...
                if "getter" in col:
                    try:
                        ...
                        logger.debug(
                            "column getter failed for %s: %s", col.get("key"), exc
                        )
                        val = ""
                else:
