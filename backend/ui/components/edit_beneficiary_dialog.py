...
class EditBeneficiaryDialog(QDialog):
    ...
    def populate_data(self):
        ...
        if idx >= 0: self.cmb_gender.setCurrentIndex(idx)
    ...
    def get_data(self):
        return {
            ...
            'weight': float(self.txt_weight.text().strip()) if self.txt_weight.text().strip() else None,
            'height': float(self.txt_height.text().strip()) if self.txt_height.text().strip() else None,
