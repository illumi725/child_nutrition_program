...
WHO_DATA = {}
try:
    data_path = os.path.join(os.path.dirname(__file__), "data", "who_standards_lms.json")
...
def calculate_anthro_stats(age_in_months, gender_str, weight, height):
    ...
    res = {
        "wfh_figure": None, "wfh_status": None,
        "wfa_figure": None, "wfa_status": None,
        "hfa_figure": None, "hfa_status": None,
        "bmifa_figure": None, "bmifa_status": None
    }
    
    if weight <= 0 or height <= 0 or age_in_months < 0: return res
    ...
    if lms:
        ...
        if z > 2: res["wfa_status"] = "OVERWEIGHT"
        elif z < -3: res["wfa_status"] = "SEVERELY UNDERWEIGHT"
        elif z < -2: res["wfa_status"] = "UNDERWEIGHT"
        else: res["wfa_status"] = "NORMAL"
    ...
    if lms:
        ...
        if z < -3: res["hfa_status"] = "SEVERELY STUNTED"
        elif z < -2: res["hfa_status"] = "STUNTED"
        elif z > 3: res["hfa_status"] = "TALL"
        else: res["hfa_status"] = "NORMAL"
    ...
    if lms:
        ...
        if z > 2: res["bmifa_status"] = "OBESE"
        elif z > 1: res["bmifa_status"] = "OVERWEIGHT"
        elif z < -3: res["bmifa_status"] = "SEVERELY WASTED"
        elif z < -2: res["bmifa_status"] = "WASTED"
        else: res["bmifa_status"] = "NORMAL"
    else:
        # Fallback for adults
        res["bmifa_figure"] = round(bmi, 2)
        if bmi < 18.5: res["bmifa_status"] = "UNDERWEIGHT"
        elif bmi < 24.9: res["bmifa_status"] = "NORMAL"
        elif bmi < 29.9: res["bmifa_status"] = "OVERWEIGHT"
        else: res["bmifa_status"] = "OBESE"
