...
def _replicate_indexes(cloud, local: sqlite3.Connection, table: str):
    ...
    for idx in indexes:
        ...
        if key_name not in index_defs:
            index_defs[key_name] = {"unique": idx["Non_unique"] == 0, "columns": []}
        index_defs[key_name]["columns"].append(
            (idx["Seq_in_index"], idx["Column_name"])
        )
    ...
    for key_name, info in index_defs.items():
        ...
        ddl = f'CREATE {unique_clause} INDEX IF NOT EXISTS "{idx_name}" ON "{table}" ({cols_str})'  # noqa: E501
        try:
            local.execute(ddl)
        except Exception as e:
...
def _create_sqlite_table_from_mysql(cloud, local: sqlite3.Connection, table: str):
    ...
    for col in cols:
        ...
        nullable = "" if col["Null"] == "NO" else ""  # noqa: F841
...
def _push_table(cloud, local: sqlite3.Connection, table: str) -> int:
    ...
    keys = list(rows[0].keys())
    update_parts = ", ".join([f"`{k}` = %s" for k in keys if k != "updated_at"])  # noqa: F841
    ...
    with cloud.cursor() as c:
        for row in rows:
            ...
            on_dup = ", ".join(
                [f"`{k}`=VALUES(`{k}`)" for k in keys if k not in where_cols]
            )
            sql = f"INSERT INTO `{table}` ({col_list}) VALUES ({placeholder_list}) ON DUPLICATE KEY UPDATE {on_dup}"  # noqa: E501
...
class SyncWorker(QThread):
    ...
    def _run_full(self):
        ...
        try:
            ...
            self.finished.emit(
                True,
                f"Successfully replicated {total_rows:,} rows from {total} tables.",
            )
        finally:
            cloud.close()
            local.close()
    ...
    def _run_sync(self):
        ...
        try:
            ...
            self.finished.emit(
                True, f"↑ Pushed {pushed_total} rows  |  ↓ Pulled {pulled_total} rows"
            )
        finally:
            cloud.close()
            local.close()
