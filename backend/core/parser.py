...
def split_beneficiary_name(raw_name, surname_dict=None):
    ...
    if len(parts) > 1 and parts[-1].upper().replace(".", "") in [
        s.replace(".", "") for s in suffixes
    ]:
    ...
    if surname_dict and middlename and middlename not in surname_dict:
        ...
        # In Philippine naming conventions, this means it's likely a compound first name.  # noqa: E501
...
def calculate_match_score(excel_rec, db_rec):
    ...
    db_ln, db_fn, db_mn = (
        clean_name(db_rec.get("lastname")),
        clean_name(db_rec.get("firstname")),
        clean_name(db_rec.get("middlename")),
    )
    ...
    names_match = (ex_ln == db_ln) and (
        (ex_fn == db_fn and ex_mn == db_mn)
        or (ex_full_fn == db_full_fn)
        or (ex_full_fn.replace(" ", "") == db_full_fn.replace(" ", ""))
    )
    ...
    if has_db_mn and not has_ex_mn:
        score -= 15
...
def get_best_site_suggestion(ex, sites_cache):
    if not sites_cache:
        return None
    ex_full = f"{ex.get('excel_site', '') or ''} {ex.get('excel_brgy', '') or ''} {ex.get('excel_city', '') or ''} {ex.get('excel_prov', '') or ''}".upper()  # noqa: E501
    best_sid, best_s_score = None, 0
    for s in sites_cache:
        db_full = f"{s.get('site_name', '') or ''} {s.get('barangay_name', '') or ''} {s.get('citymun_name', '') or ''} {s.get('province_name', '') or ''}".upper()  # noqa: E501
...
def parse_form_5a(file_path: str, surname_dict=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not form_5a_sheet:
            return records
        df_temp = pd.read_excel(
            file_path, sheet_name=form_5a_sheet, nrows=30, header=None
        )

        excel_city, excel_brgy, excel_prov, excel_site = "", "", "", ""
        for _, row in df_temp.iterrows():
            row_str = " ".join(str(v).upper() for v in row.values if pd.notna(v))
            if "CITY/ MUNICIPALITY:" in row_str:
                excel_city = str(row.tolist()[9]).strip() if len(row) > 9 else ""
            if "BARANGAY:" in row_str:
                excel_brgy = str(row.tolist()[9]).strip() if len(row) > 9 else ""
            if "PROVINCE" in row_str and "BULACAN" in row_str:
                excel_prov = "BULACAN"
            elif "PROVINCE" in row_str:
                excel_prov = str(row.tolist()[2]).strip() if len(row) > 2 else ""
            if "FEEDING SITE" in row_str:
                excel_site = str(row.tolist()[2]).strip() if len(row) > 2 else ""

        excel_batch = None
        batch_match = re.search(
            r"Batch\s*(\d+)", os.path.basename(file_path), re.IGNORECASE
        )
        if batch_match:
        ...
        if excel_batch is None:
            for _, row in df_temp.iterrows():
                ...
                if any("BATCH" in val for val in row_list):
                    idx = next(
                        (i for i, val in enumerate(row_list) if "BATCH" in val), -1
                    )
        ...
        if header_row_index == -1:
            return records
        df = pd.read_excel(
            file_path, sheet_name=form_5a_sheet, skiprows=header_row_index
        )
        ...
        birthday_col = next(
            (
                ...
                if "birthday" in str(col).lower() or "birth" in str(col).lower()
            ),
            None,
        )
        gender_col = next(
            (
                col
                for col in df.columns
                if any(k in str(col).lower() for k in ["sex", "gender"])
            ),
            None,
        )
        ...
        for col in df.columns:
            for r_idx in range(min(3, len(df))):
                ...
                if not weight_col and (
                    "weight" in val_str
                    or ("weight" in col_str and "baseline" in col_str)
                ):
                    weight_col = col
                if not height_col and (
                    "height" in val_str
                    or ("height" in col_str and "baseline" in col_str)
                ):
                    height_col = col
        ...
        for act_idx in range(start_idx, len(df)):
            ...
            lastname, firstname, middlename = split_beneficiary_name(
                raw_name, surname_dict
            )

            gender = "Boy"
            if gender_col:
                raw_g = str(row.get(gender_col)).upper()
                if "G" in raw_g or "F" in raw_g or "GIRL" in raw_g:
                    gender = "Girl"
            ...
            ex_rec = {
                "stable_id": f"{os.path.basename(file_path)}_{header_row_index + act_idx + 2}",  # noqa: E501
                ...
                "excel_brgy": excel_brgy,
                "excel_prov": excel_prov,
            ...
            if bday_str and ex_rec["weight"] and ex_rec["height"]:
                try:
                    ...
                    c_dt = (
                        datetime.datetime.strptime(date_collected, "%Y-%m-%d").date()
                        if date_collected
                        else datetime.date.today()
                    )
                    months = (c_dt.year - b_dt.year) * 12 + c_dt.month - b_dt.month
                    anthro_preview = calculate_anthro_stats(
                        months, gender, ex_rec["weight"], ex_rec["height"]
                    )
                except (ValueError, TypeError):
                    pass
...
def evaluate_match(ex, db_r, sites_cache=None):
    ...
    weight_mismatch = ex["weight"] is not None and (
        db_w is None or abs(ex["weight"] - db_w) > 0.1
    )
    height_mismatch = ex["height"] is not None and (
        db_h is None or abs(ex["height"] - db_h) > 0.1
    )
    ...
    baseline_mismatch = (
        weight_mismatch or height_mismatch or date_mismatch or birthday_mismatch
    )
