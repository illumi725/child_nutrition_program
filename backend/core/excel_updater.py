...
def update_excel_birthday(
    file_path: str, row_number: int, new_birthday_str: str
) -> bool:
    """
    ...
    try:
        ...
        # We'll scan the first 30 rows to find the header row containing "LAST NAME" or "FIRST NAME"  # noqa: E501
        ...
        for r_idx in range(1, min(31, sheet.max_row + 1)):
            row_vals = [
                str(sheet.cell(row=r_idx, column=c).value).upper()
                for c in range(1, sheet.max_column + 1)
            ]
            row_str = " ".join(row_vals)

            if "LAST NAME" in row_str or "FIRST NAME" in row_str:
                header_row_idx = r_idx  # noqa: F841
